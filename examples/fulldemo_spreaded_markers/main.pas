...
// To disable the usage of spread markers, comment the following line
{$DEFINE SPREADMARKER_USE}
...
uses
  ...
  {$IFDEF SPREADMARKER_USE},umvSpreadMarker{$ENDIF}  // spread marker will be used
  ;
...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure MapViewAfterDrawObjects(Sender: TObject);
  ...
  private
    ...
    {$IFDEF SPREADMARKER_USE}FSpreadMarkerHelper : TMapViewSpreadMarkerHelper;{$ENDIF}
    ...
    procedure OnSpreadModeEntered(Sender : TObject);
    procedure OnSpreadModeLeft(Sender : TObject);
...
begin
  ...
  FSpreadMarkerHelper.OnSpreadModeLeft:= @OnSpreadModeLeft;
  {$ENDIF}
...
var
  I: Integer;
begin
  {$IFDEF SPREADMARKER_USE}  FSpreadMarkerHelper.Free; {$ENDIF}
...
procedure TMainForm.MapViewAfterDrawObjects(Sender: TObject);
{$IFDEF SPREADMARKER_USE}
var
  i : Integer;
  ...
  pt0, pt1 : TPoint;
begin
  if not FSpreadMarkerHelper.SpreadModeActive then Exit;
  lDrawer := MapView.DrawingEngine;

  // Paint the connecting lines between the spreaded markers and the center
  lDrawer.PenColor:=clGreen;
  lDrawer.PenWidth:=1;
  for i := 0 to FSpreadMarkerHelper.SpreadMarkerCount-1 do
  begin
    pt0 := MapView.LatLonToScreen(FSpreadMarkerHelper.SpreadMarkers[i].RealPoint);
    pt1.X := pt0.X-FSpreadMarkerHelper.SpreadMarkerOfsCenterX[i];
    pt1.Y := pt0.Y-FSpreadMarkerHelper.SpreadMarkerOfsCenterY[i];
    lDrawer.Line(pt0.X,pt0.Y,pt1.X,pt1.Y);
  end;
  // Paint the markers (needed to bring them on top above other, not spreaded markers)
  for i := 0 to FSpreadMarkerHelper.SpreadMarkerCount-1 do
    MapViewDrawGpsPoint(MapView,lDrawer,FSpreadMarkerHelper.SpreadMarkers[i]);
end;
{$ELSE}
begin
end;
...
begin
  {$IFDEF SPREADMARKER_USE}
  FSpreadMarkerHelper.MapViewMouseDown(Sender,Button,Shift,X,Y);
  {$ENDIF}
end;
...
begin
  ...
  {$IFDEF SPREADMARKER_USE}
  FSpreadMarkerHelper.MapViewMouseMove(Sender,Shift,X,Y);
  {$ENDIF}
    ...
    if gpsList.Count > 0 then begin
      ...
      try
        for i:=0 to gpsList.Count-1 do
          ...
          begin
            // If the marker is spreaded, pick the original position from the helper
            // if not spreaded pick the position from the marker
           {$IFDEF SPREADMARKER_USE}

            ndx := FSpreadMarkerHelper.IndexOfMarker(TGpsPoint(gpsList[i]));
            if ndx >= 0 then
              rpt := FSpreadMarkerHelper.SpreadMarkerOrgPositions[ndx]
            else
            {$ENDIF}
              rpt := TGpsPoint(gpsList[i]).RealPoint;
            L.Add(Format('%s' + LineEnding + '  %s / %s', [
                    gpsList[i].Name,
                    LatToStr(rpt.Lat, USE_DMS),
                    LonToStr(rpt.Lon, USE_DMS)
                   ]));
          end;
...
begin
  {$IFDEF SPREADMARKER_USE}
  FSpreadMarkerHelper.MapViewMouseUp(Sender,Button,Shift,X,Y);
  {$ENDIF}
...
begin
  ...
  {$IFDEF SPREADMARKER_USE}
  FSpreadMarkerHelper.MapViewZoomChange(Sender);
  {$ENDIF}
end;
...
procedure TMainForm.OnSpreadModeEntered(Sender: TObject);
begin
  MapView.Invalidate;
end;

procedure TMainForm.OnSpreadModeLeft(Sender: TObject);
begin
  MapView.Invalidate;
end;
